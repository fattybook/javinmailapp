...
function compose_email() {
  ...
  document.querySelector('#contents-view').innerHTML = "";
...
function load_email(contents) {
  ...
  post.addEventListener('click', view_email(contents));
  console.log(post);
  if (contents.read) {
...
function view_email (contents) {
  ...
  document.querySelector('#compose-view').style.display = 'none';
  document.querySelector('#email-view').innerHTML = "";
    console.log(`This element ${contents.id} has been clicked`)
    ...
    .then(email => {
      ...
      archive_button.addEventListener('DOMContentLoaded', () => {
        'click', () => {
        archive_email(contents);
        load_mailbox("inbox");
      };
    });
